...
do {
    ...
    $projectNameInvalid = ($projectName -notmatch "^[a-z0-9]+[a-z0-9\-]*$") -or ($projectName -match "[-]{2}")
...
Write-Information "Creating new Resource Group: $rgName" -ForegroundColor Blue
$group = az group create --name $rgName --location westeurope
$groupObj = ConvertFrom-Json ([String]::Join("", $group).ToString())
Write-Information "Created new Resource Group: $($groupObj.name)" -ForegroundColor Green

Write-Information "Creating new Resource: $vaultName using id:$($userIdObj.id)"
...
Write-Information "Created new Resources: ($out)"
